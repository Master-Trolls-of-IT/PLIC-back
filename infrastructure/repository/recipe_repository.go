...
func (recipeRepo *RecipeRepo) SaveRecipe(recipe request.Recipe) (*response.Recipe, error) {
	...
	var recipeID string
	recipeInsertQuery := `INSERT INTO recipes (title, author, duration, difficulty, rating, number_of_ratings, score, kcal, icon) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id`
	if err := database.QueryRow(recipeInsertQuery, recipe.Title, recipe.UserEmail, recipe.Duration, recipe.Difficulty, 0, 0, 0, 0, "").Scan(&recipeID); err != nil {
		return nil, err
	}
	responseRecipeID, _ := strconv.Atoi(recipeID)
	// Insert recipe ingredients
	recipeIngredientInsert := `INSERT INTO recipe_ingredient (recipe_id, label)
    														SELECT $1, ingredient.id, $2 FROM ingredient WHERE ingredient.label = ANY($3::TEXT[])`
	if _, err := database.Exec(recipeIngredientInsert, responseRecipeID, pq.Array(ingredientLabels)); err != nil {
		return nil, err
	}
	...
	recipeStepsInsert := `INSERT INTO recipe_step (recipe_id, step_number, description)
    						SELECT $1, step.id, $2 FROM step WHERE step.description = ANY($3::TEXT[])`
	if _, err := database.Exec(recipeStepsInsert, responseRecipeID, pq.Array(recipe.Steps)); err != nil {
		return nil, err
	}
	// Retrieve the recipe
...
func (recipeRepo *RecipeRepo) UpdateRecipe(recipeID int, recipe request.Recipe) (*response.Recipe, error) {
	...
	// Update the recipe
	recipeUpdateQuery := `UPDATE recipes SET title = $1, author = $2, duration = $3, difficulty = $4 , rating = $5, number_of_ratings = $6, score = $7, kcal = $8, icon = $9 WHERE id = $10`
	if _, err := database.Exec(recipeUpdateQuery, recipe.Title, recipe.UserEmail, recipe.Duration, recipe.Difficulty, 0, 0, 0, 0, "", recipeID); err != nil {
		return nil, err
	}
	...
	recipeIngredientInsert := `INSERT INTO recipe_ingredient (recipe_id, label)
    														SELECT $1, ingredient.id, $2 FROM ingredient WHERE ingredient.label = ANY($3::TEXT[])`
	if _, err := database.Exec(recipeIngredientInsert, recipeID, pq.Array(ingredientLabels)); err != nil {
		return nil, err
	}
	...
	// Insert recipe steps
	recipeStepsInsert := `INSERT INTO recipe_step (recipe_id, step)
    						SELECT $1, $2 FROM UNNEST($3::TEXT[]) step`
	if _, err := database.Exec(recipeStepsInsert, recipeID, pq.Array(recipe.Steps)); err != nil {
		return nil, err
	}
	// Retrieve the recipe
